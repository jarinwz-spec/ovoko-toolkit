...
// ---- felgi (alloy) i opony po ROZMIARZE (kategoria z tytulu; przed regulami JSON) ----
function resolveWheelTire(title){
  const T=norm(title);
  const RIM={12:'468',13:'491',14:'492',15:'493',16:'682',17:'683',18:'684',19:'685',20:'686',21:'687',22:'688',23:'1885',24:'1886'};
  const TS={13:'471',14:'472',15:'494',16:'495',17:'691',18:'692',19:'693',20:'694',21:'695',22:'696'};
  const TW={13:'712',14:'713',15:'714',16:'715',17:'716',18:'717',19:'718',20:'719',21:'720',22:'721'};
  function dia(){
    let m=T.match(/(\d{3})\s*[\/X]\s*(\d{2})\s*R?\s*(\d{2})/); if(m)return +m[3];
    m=T.match(/\bR(\d{2})\b/); if(m)return +m[1];
    m=T.match(/[X\u00D7]\s*(\d{2})\s*["\u201D\u2033]/); if(m)return +m[1];
    m=T.match(/\d+[.,]?\d*\s*J?\s*[X\u00D7]\s*(\d{2})/); if(m)return +m[1];
    return 0;
  }
  if(/\bOPON[AY]\b/.test(T)){
    const d=dia(); if(!d)return null;
    const winter=/ZIMOW|WINTER|BLIZZAK|SNOW|ALPIN|ICE|WINTERCONTACT|LM005/.test(T);
    return (winter?TW:TS)[d]||null;
  }
  if(/\bFELG[AI]\b|OBRECZ|ALUFELG/.test(T)){
    const d=dia(); return d?(RIM[d]||null):null;
  }
  return null;
}
...
for(const it of pkg){
  ...
  let catId=resolveWheelTire(it.title)||resolveCat(it.title);
